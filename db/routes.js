...
router.post('/admin/inventario',
  ...
  [
    ...
    body('cuentas.*.perfil').optional({ nullable: true }).isString().trim().isLength({ max: 100 }),
    body('cuentas.*.pin').optional({ nullable: true }).isString().trim().isLength({ max: 50 }),
    body('cuentas.*.servidor').optional({ nullable: true }).isString().trim().isLength({ max: 500 }),
    body('cuentas.*.notas').optional({ nullable: true }).isString().trim().isLength({ max: 1000 }),
  ],
  validar,
  async (req, res) => {
    try {
